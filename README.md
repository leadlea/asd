...
| **宗田さんTexレビュー round（2026-07-03）** | ⏳ 対応中 | 宗田さんが `paper1_ja_st.tex`（構成再編＋[CHECK]/[NOTE]コメント埋め込み）を差し戻し。当面は構成を維持しつつ[CHECK]対応。まず数値整合・書式の機械的3件（#14本文相関値の集計元整合／#15表添字ローマン体／#26教師一致度図の再生成）を完了（[チェックリスト](docs/homework/soda_check_list_20260703.md)）。残りの[CHECK]と背景・考察のブラッシュアップは継続 |
| **編集プラットフォーム** | ✅ 確定 | Overleaf移行は取りやめ。gitベース（`paper1_ja_st.tex` を追跡）で継続。宗田さんとはdiffでやり取り |
...
> **残タスク**: 残りの[CHECK]（表記統一・図の凡例位置・当たり前手続きの削除要否など約26件）と、全体相談事項（3段階リッジの掲載可否／C次元のみ報告→全次元掲載／ベースライン・Dose-Responseの本文or補足）、背景・考察のブラッシュアップ。
...
| [論文ドラフト（宗田さんレビュー版）](paper1_ja_st.tex) | 🆕 宗田さんが構成再編＋[CHECK]/[NOTE]コメントを埋め込んだ現行作業版。当round中はこちらで編集し、diffで宗田さんとやり取り |
...
├── paper1_ja_st.tex           # 宗田さんレビュー版（構成再編＋[CHECK]/[NOTE]、当round作業版）
