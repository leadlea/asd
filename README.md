...
> **注（2026-06-22）**: 本文は主指標を相関r→増分妥当性R²(OOF-pooled)/RMSEへ変更済み。上表のr版は付録に保持。R²/RMSEではNovel追加(Stage2→3)で4次元(O/C/A/N)が改善側に評価されるが、paired bootstrap検定では増分はいずれも有意でない（p=0.23–0.58、Eのみ有意に悪化 p=0.030）。Novel特徴量の実質寄与は係数レベル（Permutation回帰係数検定）で担保。
