...
| **宗田Texレビュー round（2026-07-03）** | ⏳ 対応中 | 宗田さんが `paper1_ja_st.tex`（構成再編＋[CHECK]/[NOTE]コメント埋め込み）を差し戻し。当面は構成を維持しつつ[CHECK]対応。まず数値整合・書式の機械的3件（#14本文相関値の集計元整合／#15表添字ローマン体／#26教師一致度図の再生成）を完了（[チェックリスト](docs/homework/soda_check_list_20260703.md)）。残りの[CHECK]と背景・考察のブラッシュアップは継続 |
...
### 2026-07-03 修正（宗田Texレビュー round・数値整合/書式）
...
| [論文ドラフト（宗田レビュー版）](paper1_ja_st.tex) | 🆕 宗田さんが構成再編＋[CHECK]/[NOTE]コメントを埋め込んだ現行作業版。当round中はこちらで編集し、diffで宗田さんとやり取り |
...
| [宗田Texレビュー対応チェックリスト（2026-07-03）](docs/homework/soda_check_list_20260703.md) | 🆕 `paper1_ja_st.tex` 埋め込みの[CHECK]29件/[NOTE]の対応管理表＋作業ログ＋図表↔解析ファイル対応表。差分パッチ `soda_diff_20260703.patch` 併置 |
...
├── paper1_ja_st.tex           # 宗田レビュー版（構成再編＋[CHECK]/[NOTE]、当round作業版）
