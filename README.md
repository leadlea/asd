...
| **宗田さん2ndレビュー round（2026-07-06）** | ✅ 完了・お礼/進捗報告予定 | 宗田さんが言い回しを微調整＋新規[CHECK]/[NOTE]を付けた `paper1_ja_st_rev_20260705_st.tex` を差し戻し。Plan Aで対応：**topic_drift残存が係数検定/Bootstrap結果に混入（20特徴量）し本文の19特徴量記載と矛盾**していた問題を、ensembleで19特徴量再計算して解消。Cのドライバが旧7個（pause系優勢）→**新5個**（係数検定とBootstrap CIが同一5個で完全一致）。[CHECK-A]数値をmetadataと突合し全一致、[CHECK-B]§2.2の新旧1対1非対応を明記、DeepSeek 671Bパラメータ、[NOTE]編集経緯ログ26件削除。返却版 `paper1_ja_st_rev_20260706.tex`（43ページ）。詳細は[チェックリスト](docs/homework/soda_check_list_20260703.md) |
...
### 2026-07-06 修正（宗田さん2ndレビュー round・topic_drift完全削除の再解析）
