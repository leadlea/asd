...
### 3.3 関連背景：語用論的推論における性差・補償戦略
> 「談話 × LLM × ASD推定」の直接系譜ではないが、**性差をサブグループで分けて見る設計**と
> **「行動は定型に近いが処理コストは高い＝補償戦略」**という解釈枠組みは、本研究の
> 相互行為特徴量・性差の議論に直接効く。山下先生からの共有（2026-06）で追加。

#### [BG-1] Porrini, Goulston et al.（2026, *Journal of Autism and Developmental Disorders*）
- タスク: スカラー含意（語彙的・アドホック）の導出を**プライミング課題**で測定
- 設計: 自閉成人52 + 非自閉成人52（**性別バランス済**）。協力的・知識のある「対話者」の
  **存在/不在**を被験者間条件に。二次解析で**性別を固定因子**に追加し、反応時間も混合モデルで分析
- 主結果:
  - プライミング効果は群・含意タイプを通じて生じ、自閉成人も文脈依存的に柔軟解釈
  - **性別を入れて初めて自閉群内の差が顕在化**：自閉女性は非自閉群と同様に話者存在下で含意導出が増えるが、**反応時間は長い**。自閉男性は逆に話者存在下で含意導出が減る
  - 解釈：自閉女性は**コストの高い補償戦略**で定型と同等の解釈に到達。自閉男性は心的状態推論の追加負荷の影響を強く受ける
- 本研究への含意:
  - 性差を交絡として均すのでなく**分けて解析する根拠**として引用可
  - 相互行為特徴量（フィラー・応答型・修復）を**補償行動の定量指標**として位置づける議論に接続
  - 引用候補として den Hartog 2023（性差レビュー）/ Hull 2020（カモフラージュ）/ Banker 2025（自己申告高特性≠臨床診断）を二次取得できる

Ref:
- DOI: https://doi.org/10.1007/s10803-026-07420-2
...
[BG: 語用論的推論の性差・補償戦略 (J Autism Dev Disord, 2026)]
- Porrini, Goulston et al. 2026 (DOI)
  https://doi.org/10.1007/s10803-026-07420-2
