...
2. **2段階の妥当性検証設計**: コーパス基本情報（年齢）による表面的妥当性 → Big5による構成概念妥当性
...
特徴量の妥当性を外部指標から検証するため、コーパスに付随する話者属性情報（年齢）と18特徴量の関連を分析した。
...
|--------|----------:|----:|-----------:|----:|------|
| PG_resp_gap_p50 | 0.246 | 0.013 | 0.263 | 0.008 | 年齢が高い話者ほど応答遅れが長い → 加齢に伴う処理速度の低下、慎重な応答スタイル |
| IX_oirmarker_after_question_rate | 0.211 | 0.033 | 0.250 | 0.011 | 年齢が高い話者ほど質問直後のOIR（聞き返し）が多い → 聴覚的処理負荷の増大、確認行動の増加 |

その他の16特徴量については、年齢との有意な相関は認められなかった（p > 0.05）。

> **注**: 本コーパス（CEJC home2 HQ1）では性別情報が利用不可であったため、性別による群間比較は実施できなかった。

**妥当性の含意**: これらの結果は、提案特徴量の一部が話者の人口統計学的属性と理論的に整合する関連を示すことを確認するものであり、特徴量の**表面的妥当性（face validity）**を支持する。
...
**注目すべき交差的知見**: IX_oirmarker_after_question_rateは、3.3節で年齢と有意な正の相関を示すと同時に、CのBootstrap Top Driverでもある。修復開始行動が認知的側面（加齢に伴う処理負荷）と性格的側面（誠実性に関連する確認行動）の両方を反映する多面的な指標である可能性を示唆する。
...
- **PG_resp_gap_p50 × 年齢**: 年齢が高い話者ほど応答遅れが長い → 加齢に伴う処理速度の低下として直感的に理解しやすい。**査読者にとって最も「納得感」のある結果**。
- **IX_oirmarker_after_question_rate × 年齢**: 年齢が高い話者ほど聞き返しが多い → 聴覚的処理負荷の増大として解釈可能。
- **性別データの不在**: CEJC home2 HQ1では性別情報が利用不可。今後、性別情報を含むデータセットでの追試が望まれる。
...
- **性別データの不在**: 性別は会話スタイルに影響を与える重要な社会言語学的変数。今後の追試が必要。
- **次ステップ候補**:
...
| コーパス基本情報の活用 | 3.3節、論文Results 3.3 | 年齢との関連分析を追加。PG_resp_gap_p50×年齢、IX_oirmarker_after_question_rate×年齢の有意な相関を報告 |
| 新規図表の追加 | 3.1〜3.3節 | 特徴量分布図、相関ヒートマップ、年齢×特徴量散布図を追加 |

### 5.3 次回NCNPミーティングでの確認事項

1. **新Results構成の妥当性**: 4段構成（記述統計→相関→コーパス基本情報→Big5）の流れは適切か？
2. **コーパス基本情報分析の深度**: 年齢のみの分析で十分か？ 他の属性情報（出身地域・学歴等）の入手可能性は？
3. **Big5分析の位置づけ**: 「構成概念妥当性の検証」という立て付けで査読に耐えるか？
4. **OIR指標の多面性**: IX_oirmarker_after_question_rateが年齢とCの両方と関連する知見の解釈について
5. **対象学会の最終決定**: 発達支援学会（2026年11月）で確定してよいか？
