...
| 12 | 374 | 結果・相関分析 | 多重共線性/相関1の記述削除検討 | ✅ | 宗田さん指摘に同意し**削除確定**<br>「相関分析節でRidge正則化/多重共線性を語るのは流れが悪い」→該当文（IX_lex_overlap×topic_drift r=−1.00、既にコメントアウト済み）を[CHECK]ごと除去<br>本文3.2の現行テキストに多重共線性/Ridgeの記述は無く整合（r=−1.00の扱いは#6で2.4に集約済み） |
...
| 23 | 1237 | 補足・感度分析 | 「特徴量サブセットの感度」記述の適否 | ✅ | 宗田さん判断に同意：完全共線変数(IX_topic_drift_mean)除外の記述は感度分析でなく前処理（#6で方法2.4に集約済み）→コメントアウト削除で確定<br>加えて見出し「特徴量サブセット」が中身（交絡統制）と不一致→中身に合わせ**「交絡変数（性別・年齢）の統制」に改名**<br>解決済み[CHECK]2件削除 |
...
- **#12 相関分析の多重共線性/相関1記述**: 宗田さん指摘「相関分析節でRidge正則化・多重共線性を語るのは流れが悪い」に同意し削除確定。該当文（`IX_lex_overlap_mean`×`IX_topic_drift_mean` の $r=-1.00$、既に`% >`でコメントアウト済み）を[CHECK]ごと除去。現行3.2本文に多重共線性/Ridgeの記述はなく整合（完全共線の扱いは#6で方法2.4に集約済み）。
- 検証: `uplatex paper1_ja_st.tex` exit 0（46ページ）。一時ファイル削除済み。
...
- **#19 「推定が不安定」表現（付録A.1 選定基準）**: 宗田さん指摘（基準を設けなくても特徴量の計算自体は安定して可能、ゼロ除算を除く）に同意。実態は「少数観測に基づく比率推定の標本変動が大きくなる」であり、計算の不安定さではない。
...
- **#23 「特徴量サブセットの感度」記述の適否**: 宗田さんがコメントアウトした「完全共線変数(IX_lex_overlap_mean/IX_topic_drift_mean)のうちtopic_driftを除外」の記述は、感度分析ではなく前処理（#6で方法2.4に集約済み）。宗田さん判断に同意し**削除確定**。
