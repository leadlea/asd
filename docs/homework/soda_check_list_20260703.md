...
|---|-----|------|------|------|------|
| 1 | 155 | 方法・会話データ | レコード280想定に対しズレる理由の説明 | ✅ | 実データで確認：CEJC577件中「自宅」=**140件**（話者数問わず）、自宅かつ2名は88件<br>展開後378は自宅140（多人数含む）を会話×話者に展開した数で再現（88なら178）<br>本文の「自宅かつ2名（140）」を「自宅（140）」に修正し、多人数会話を含むため280でなく378になる旨を明記<br>2者間性は最終セットの性質として後段に記述 |
