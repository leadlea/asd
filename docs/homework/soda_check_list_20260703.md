...
> 「理由・意思決定」列は横スクロール防止のため1行要約に留める。完了項目の詳細な根拠・修正箇所・検証は本ファイル下部の「作業ログ」に記載する。

| # | 行 | カテゴリ | 内容（要約） | 状態 | 理由・意思決定（要約／詳細は作業ログ） |
|---|-----|------|------|------|------|
| 1 | 155 | 方法・会話データ | レコード280想定に対しズレる理由の説明 | ⬜ | — |
| 2 | 157 | 方法・会話データ | 発話タイミング情報の出所・重要性・欠損の説明 | ⬜ | — |
| 3 | 167 | 方法・会話データ | 「66会話中2会話」の66会話とは何か | ⬜ | — |
| 4 | 175 | 方法・特徴量 | 削除変数含め合計18種になるか確認 | ✅ | 説明変数は19種（Classical10＋Novel9、PG=9）。実装3本・定義表・本文と一致。L177/L181/L231を19基準に統一 |
| 5 | 189 | 方法・特徴量 | 性別・年齢の相関分析の欠損補完を加筆 | ✅ | 関連分析はpairwise deletion（補完なし）。回帰の中央値補完と区別して2.2に明記 |
| 6 | 233 | 方法・回帰分析 | 相関1の変数を一方の指標に統一 | ✅ | 完全共線はIX_topic_drift_meanのみ。IX_lex_overlap_meanに統一済み、2.4に明示 |
| 7 | 234 | 方法・回帰分析 | 「除外された変数」の言及確認・説明or削除 | ✅ | EXCL3統制変数は非説明変数。cryptic羅列を概念的1文に置換 |
| 8 | 243 | 方法・回帰分析 | 予測指標の説明を方法へ・Slack齟齬確認 | ✅ | r/R²/RMSE（OOF連結）は方法2.4で整合確認。RMSE主指標化は保留（[NOTE]維持） |
| 9 | 249 | 方法・回帰分析 | Bootstrap検定の記述を加筆 | ✅ | paired bootstrap詳細を2.4加筆。誤差・リサンプル単位を「被験者→レコード」に修正 |
| 10 | 314 | 結果・記述統計量 | 特徴量の英/日表記を統一 | ✅ | 正式参照＝コード識別子に統一。定義は`code（日本語名）`順 |
| 11 | 315 | 結果・記述統計量 | PG系分布の主張を具体値の根拠付きで修正 | ⬜ | — |
| 12 | 374 | 結果・相関分析 | 多重共線性/相関1の記述削除検討 | ⬜ | — |
| 13 | 406 | 結果・属性情報との関連 | 表Feature列を2行化・添字削除 | ✅ | Gender/Age見出しで2行化、添字簡素化。生成元も修正 |
| 14 | 505 | 結果・性格特性との関連 | 本文と表で数字が異なる点を確認 | ✅ | 集計元TSVを正として本文値を修正。順位A>C>O>N>Eは不変 |
| 15 | 506 | 結果・性格特性との関連 | 表r/p添字を斜体→ブロック体 | ✅ | ラベルのため`\mathrm`（ローマン体）に。生成元も修正 |
| 16 | 574 | 結果・予測に重要な特徴量 | 表はチェックマーク不要（太字のみ） | ✅ | Sig.チェックマーク列を削除、有意は太字。数値不変 |
| 17 | 606 | 結果・予測に重要な特徴量 | 図の凡例位置調整 | ✅ | fig_bootstrap_varianceの凡例を軸外へ移動。生成元も修正 |
| 18 | 641 | 考察 | 考察を一般的な構造に修正 | ⬜ | — |
| 19 | 992 | 補足・付録A(会話データ) | 「推定が不安定」表現の補足・修正 | ⬜ | — |
| 20 | 1010 | 補足・付録A(会話データ) | 当該セクション説明の要否 | ⬜ | — |
| 21 | 1049 | 補足・LLM仮想Big5 | 各モデルの記述粒度を統一 | ✅ | Wright2026準拠で公開区分表記に統一。本文・図ラベルも統一し再生成 |
| 22 | 1232 | 補足・感度分析 | 補足として提示するなら結果も示す | ⬜ | — |
| 23 | 1237 | 補足・感度分析 | 「特徴量サブセットの感度」記述の適否 | ⬜ | — |
| 24 | 1241 | 補足・感度分析 | 3段階リッジとの差異を明確化 | ⬜ | — |
| 25 | 1311 | 補足・LLMモデル間の差異 | Big5次元の呼称を統一 | ✅ | 5次元一律で英語スキームに統一（初出フルネーム＋略字→以降略字） |
| 26 | 1331 | 補足・LLMモデル間の差異 | 本文値と図の値の不一致を確認 | ✅ | 本文は正。旧版図を再生成し0.699表示に統一 |
| 27 | 1350 | 補足・LLMモデル間の差異 | C因子の粒度／頑健性主張の方針 | 💬 | 全体相談待ち。全次元掲載か頑健性主張かはMTGで決定、原稿修正は保留 |
