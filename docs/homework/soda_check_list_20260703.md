...
|---|-----|------|------|------|------|
| 1 | 155 | 方法・会話データ | レコード280想定に対しズレる理由の説明 | ✅ | ズレの原因は数字の**単位の混同**（140は人数でなく**会話数**）。実データで確認した流れ:<br>・CEJC全**577会話**→収録場所「自宅」の**会話が140件**（話者数問わず。自宅かつ2名は88件）<br>・会話×話者に展開→**378レコード**（自宅は3名以上の多人数会話も含むため140×2=280を超える。88経路なら178で不一致）<br>・HQ1品質基準で258除外→**120レコード**＝**66会話・74話者**（実人数74名）<br>本文の「自宅かつ2名（140）」を「自宅（140件）」に修正し、280でなく378になる理由を明記。2者間性は最終セットの性質として後段に記述 |
| 2 | 157 | 方法・会話データ | 発話タイミング情報の出所・重要性・欠損の説明 | ✅ | 発話開始・終了時刻はCEJC発話単位データに**元来付与**（PG系特徴量の算出に使用）と明記<br>258の除外はHQ1品質基準によるもので「タイミング対応づけによる欠落ではない」と補足 |
| 3 | 167 | 方法・会話データ | 「66会話中2会話」の66会話とは何か | ✅ | 実データで確認：120レコードは**66会話・74話者**に由来<br>品質基準はレコード（会話×話者）単位のため2名とも通過=54会話／片方のみ=12会話<br>→会話数66はレコード数の半分60と一致しない旨を明記 |
...
- **確定した数値連鎖**（すべて再現済み）:
  ...
  - HQ1品質基準（n_pairs≥80／text_len≥2000／q_pairs≥10）で258除外 → **120レコード**
...
- **#2**: 発話開始・終了時刻はCEJC発話単位データに元来付与（PG系特徴量に使用）と明記。「タイミング対応づけでデータが失われたのでは」への回答として、258除外はHQ1品質基準のみによる旨を補足。
- **#3**: 120→66会話・74話者。品質基準がレコード単位のため1会話から2名通過(54)/片方のみ(12)が生じ、会話数66はレコード数の半分60と一致しない旨を明記。
