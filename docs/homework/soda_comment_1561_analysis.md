...
- Method記述の精緻化＋図の再生成で解決可能。**推奨は案A（図をStage 3と同じ21 features + fold_meanに統一）**。

---

## 1. 検証の経緯

`paper1_ja_st.docx` に寄せていただいたレビューコメントのうち、表6と散布図の数値が一致しない旨のご指摘（#1561）について、原因を切り分けるため定量的な検証を行った。
...
- 散布図は paper-finalization 作業時に「相互行為特徴量による予測の可視化」として別途追加した経緯があり、Stage 3 との特徴量セット・集約方法の整合性チェックが漏れていた。今回のご指摘で発覚。
...
1. 表6 の公表値をそのまま保持できる
2. 修正コストが最小（1関数のみ）
3. 散布図の位置づけが「Stage 3 モデルの予測精度の可視化」として整合する
4. Method 記述の整合性も保ちやすい

最終決定は週末のレビュー結果を踏まえて確定させたい。

---

## 6. 案A実行時のアクションリスト

レビュー後、方針が確定した場合の実行手順（参考）。
