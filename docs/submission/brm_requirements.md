...
5/10 の初版以降、日本語版 paper1_ja.tex の仕上げで以下を実施し、BRM 要件への適合度が向上した。

- **図表をBRM体裁に調整**: 図の解像度を引き上げ（主要図 dpi=300〜600）、図4(3段階Ridge)を主指標R²版に修正、表9(19×19相関行列)の行ラベルを番号化してフォント可読性を改善。中身（数値・結論）は不変。
- **書式統一（太字）**: 本文の強調太字 `\textbf` を全36箇所除去し書式を統一。見出しの構造的太字とアスタリスクによる有意表記は維持（APA/BRM の本文強調＝原則イタリック方針とも整合）。
- **参考文献の正確性チェック完了**: 全20件を CrossRef 等で照合し、書誌の誤り3件を実在文献に訂正。
  - Kita & Ide 2007: 誌名/巻号/副題を訂正（Journal of Pragmatics, 39(7), 1242–1254, DOI 10.1016/j.pragma.2007.02.009）
  - 「自閉症者の相槌多様性」引用: 著者誤り → Wehrle, Vogeley, & Grice (2024), Language and Cognition, 16(1), 108–133, DOI 10.1017/langcog.2023.21
  - 実在未確認だった Meylan & Gahl 2014 → Brennan & Clark (1996), JEP:LMC, 22(6), 1482–1493 に差替
  - Wright et al. (2026) は CrossRef で実在確認済み（Nature Human Behaviour, DOI 10.1038/s41562-025-02389-x）
- **本文の見せ方（参考）**: 3.4.3 を増分妥当性R²/RMSE主指標＋paired bootstrap検定へ刷新、3.4.6 教師間一致度を付録へ移設（いずれも査読耐性・誠実な立て付けを優先）。

> これらにより下表の「Figure 解像度」「Reference style/正確性」「Manuscript 形式（強調）」は対応が進んだ。残課題は英訳・APA7 bib 化・Declarations 追記・double-spaced 化など（英訳フェーズで実施）。
...
- **対策**: 英訳時に以下のカテゴリで引用を追加検討
  - LLM-based assessment 系（既に Hu 2025, Wright 2026, Mun 2024 等はあり、1〜2件追加）
...
| 2026-06-25 | 図表のBRM体裁化（dpi向上・図4 R²化・表9可読性）・本文太字除去・参考文献の正確性チェック完了（誤り3件訂正）を反映 | 福原 |
