...
drop_cols = [col for col in d.columns if col.startswith('input_') or col.startswith('child9_') or col=='T2_child_word_types']
base_cols = [col for col in d.columns if col not in drop_cols]
out = d[base_cols].copy()

need_p = [x for x in ['dyad_id','input_word_tokens','input_word_types','input_num_utterances','input_mlu'] if x in p2.columns]
need_c = [x for x in ['dyad_id','child9_ndw_strict','child9_tokens_strict','child9_utts_strict'] if x in c2.columns]
...
# 数値化
for col in ['input_word_tokens','input_word_types','input_num_utterances','input_mlu']:
...
if 'diagnostic_group' in out.columns:
    for col in ['input_word_tokens','input_mlu','T2_child_word_types']:
        if col in out.columns:
            print("\n[SUMMARY]", col)
