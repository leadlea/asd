...
% TODO: tab_ensemble_permutation.tex が生成され次第，以下のコメントを解除
% \begin{table}[htbp]
% \centering
...
\end{tabular}
\par\smallskip
{\footnotesize\textit{注: アンサンブル分析の実行後に実数値で更新予定．}}
\end{table}

Conscientiousness（C: 誠実性）はアンサンブルにおいても最も頑健な結果を示し，
5次元中で最も高い予測精度と最も低い$p$値を記録した．
この結果は，Cの予測可能性が特定のLLM教師に依存しない頑健な知見であることを
アンサンブルの観点からも裏付けるものである．

% TODO: fig_ensemble_permutation.png が生成され次第，以下のコメントを解除
% \begin{figure}[htbp]
% \centering
...
A, E, N, Oの各次元については，アンサンブルにおける有意性の変化を
次項（3.4.2節）で個別教師結果と比較して報告する．
...
\toprule
次元 & $r_{\text{baseline}}$ & $p_{\text{baseline}}$ & $r_{\text{extended}}$ & $p_{\text{extended}}$ & $\Delta r$ \\
\midrule
O & --- & --- & --- & --- & --- \\
C & --- & --- & --- & --- & --- \\
E & --- & --- & --- & --- & --- \\
...
\end{tabular}
\par\smallskip
{\footnotesize\textit{注: ベースラインvs拡張比較分析の実行後に実数値で更新予定．}}
\end{table}

% TODO: fig_baseline_vs_extended.png が生成され次第，以下のコメントを解除
% \begin{figure}[htbp]
% \centering
...
% --- 交絡統制結果のプレースホルダー ---
% TODO: confound_analysis.pyの実行結果に基づき，以下のいずれかのパターンで記述を更新する
%
% パターン1: Cが交絡統制後も有意な場合
%   → 「性別・年齢を統制した後もCの有意性は維持された（Model B: r = X.XXX, p = X.XXXX）．
%      これは，特徴量とCの関連が性別・年齢の影響を超えた独自の寄与を持つことを示し，
%      提案特徴量の構成概念妥当性を追加的に支持する根拠となる．」
%
% パターン2: Cの有意性が低下・消失した場合
%   → 「性別・年齢を統制した後，Cの予測精度は低下した（Model A: r = X.XXX → Model B: r = X.XXX）．
%      この結果は，特徴量とCの関連の一部が性別・年齢を介した間接的なものである可能性を示唆する．
%      ただし，Ridge回帰において説明変数を追加した場合の精度変化は，
%      交絡の存在のみならず，説明変数間の共線性増大による推定不安定化も反映しうるため，
%      解釈には注意が必要である．」
...
$p$値の変化を報告する．
% TODO: confound_analysis.pyの実行結果に基づき，具体的な数値を記入予定
