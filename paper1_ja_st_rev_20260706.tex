...
\title{日本語日常会話における相互行為特徴量の定量化指標の提案とその妥当性検証}
...
IX: 連鎖組織，RESP: 応答型）を定量化指標として提案する．
その妥当性を，まずコーパス基本情報（性別・年齢）との関連から，
次に外部基準の一つとしてBig5性格特性との関連から段階的に検証する．
...
さらに，外部基準の一つとして4つの大規模言語モデル（LLM）が推定した
Big5スコアを用い，Ridge回帰（5-fold CV, GroupKFold subject-wise split）と
置換検定（5000回）を適用した．
その結果，Big5のうち Openness（O），Conscientiousness（C），
Agreeableness（A），Neuroticism（N）の複数次元が
提案特徴量と有意に関連し，これらの指標が話者の性格に関連する情報を
含むことを確認した．
...
採点させ，その平均が自己報告のBig5と収束的妥当性を
示すことを報告している．
これらの研究はLLM/PLMの高い推定能力を示す一方で，
共通する課題として説明可能性の欠如が挙げられる．
...
これらの多くは音声研究・談話分析・NLPで広く用いられてきた指標であり\cite{levitan2012,deruiter2006,clark1996,watanabe2003}，
先行研究との比較可能性を担保する．
...
計19の相互行為特徴量を定量化指標として提案する．
本研究の主たる貢献は特徴量の提案そのものにあり，
Big5性格特性との関連分析は，提案指標の構成概念妥当性を
検証するための手段として位置づけられる．
...
録場所が「自宅」である会話を分析対象として抽出した（140件）．自宅を選定したのは，
比較的リラックスした環境で収録されており話者の自然な会話行動が反映されやすいため
である．

% [NOTE] このセクションに記載されていた交差検証の話は，このデータセットの話とは直接の関係がないため，交差検証の方に記載するのが望ましいです．また重複もあり冗長な内容になっています．

次いで，各会話を会話$\times$話者のレコードに展開し，378件のデータを構成した。各レコードには，転記テキストだけでなくCEJCの発話単位データに元来付与されている発話開始・終了時刻などが紐づけられている。各話者の相互行為特徴量を安定して推定できるだけの会話量を確保するため，(i) 発話ペア数（$\geq 80$），(ii) テキスト文字数（$\geq 2{,}000$字），(iii) 質問直後ペア数（$\geq 10$）をすべて満たすレコードのみを採用した．この選定基準により258レコードが除外され，最終的に120レコードが分析対象となった．

% また，加筆いただいた情報ですが，文章のフローを考慮して，削除したり最後の段落に補足的な情報として記載したりするように，調整いたしました。
...
最終的な120レコードは，66会話・74話者に由来した．CEJCでは同一話者が複数の会話に参加する場合があり，74名のうち25名が2件以上の会話に参加していた（重複話者に属するレコードは71件，全体の59.2\%）．残りの49名は各1件の会話にのみ参加している．話者74名の性別内訳は，女性38名・男性36名であり，レコード単位では女性66件・男性54件となる．最終的な分析対象は，いずれも2者間会話に由来していた。なお，CEJCに付与されたメタデータ上の話者数が2名であっても，実際の発話データでは一時的に第三者が参加している会話が少数含まれる（66会話中2会話）．これらの会話では，メタデータに記載された話者2名の発話ペアのみを特徴量計算に使用した．

CEJCメタデータ（性別・年齢等の話者属性）の紐付け手順，各基準の閾値の根拠と算出法は\ref{sec:supp_dataset}に詳細を示す．
...
会話に現れる二者間の相互行為を定量化することを目的に19の特徴量を構築した．表\ref{tab:feature_def}に各特徴量の名称，カテゴリ，分類（Classical/Novel），概要，計算アルゴリズムを示す．
...
カテゴリの区分と新旧（Classical/Novel）の区分は独立しており，1対1には対応しない．タイミング系（PG，9変数）は，発話や沈黙・応答の間合いを捉える指標のうち8変数が音声研究・談話分析・NLPで広く用いられてきた既存指標であり，これに沈黙長の変動係数（PG\_pause\_variability，1変数）を本研究で新たに定義して加えた．フィラー系（FILL，2変数）はいずれも既存研究で用いられてきた指標である．一方，連鎖組織系（IX，5変数）と応答型系（RESP，3変数）は全変数が本研究で新たに定義した指標であり，修復連鎖の組織化\cite{schegloff1977}，隣接ペアの選好構造\cite{sacks1974}，終助詞の語用論的機能といった会話現象を定量化可能な指標に変換したものである．

なお各特徴量は，予測妥当性の段階的検証（\ref{sec:pred_personality}節の3段階Ridge回帰）のため，既存研究に基づく古典的特徴量（Classical，10個）と本研究で新たに定義した新規特徴量（Novel，9個）にも区分される（表\ref{tab:feature_def}のClass列）．

なお，一部の特徴量は，計算途中において分母がゼロとなり欠損値となる場合がある（例:IX\_oirmarker\_after\_question\_rateは質問直後の発話ペアが0件の場合には欠損値となる）．
...
提案特徴量の妥当性を検証するため，話者の属性情報（性別・年齢）と相互行為特徴量の関連を分析した．性別については，2群間の分布の差をMann-Whitney $U$検定（両側検定）により評価した．年齢については，Spearman順位相関係数$\rho$を算出した．Spearmanを採用した理由は，特徴量の多くが正規分布から外れるためである．有意水準は$\alpha = 0.05$とした．欠損値が生じたレコードは除外して分析を行った．したがって関連分析における各特徴量の有効標本サイズは特徴量ごとに異なりうる．
...
相互行為特徴量の有用性を分析するために，回帰分析では説明変数を3段階に分け順次追加投入した．Stage 1は人口統計学的変数のみ（性別・年齢の2変数），Stage 2はStage 1に古典的特徴量（タイミング系8個 +フィラー系2個）を加えた12変数，Stage 3はさらに新規特徴量（相互行為系5個 + 応答系3個 + PG\_pause\_variability）を加えた全21変数である．回帰分析では\ref{sec:features}節で定義した19の相互行為特徴量を説明変数として用いる．
...
IX系では，IX\_lex\_overlap\_mean（$U = 1109$, $p = 0.0004$）は女性の方が語彙重なりが高かった．
% [山下6/22適用] #14=C: 性差の解釈は考察（コーパス基本情報との関連の解釈）に移動済みのため，結果節からは数値のみ残し解釈文を削除．

13の特徴量が年齢と有意に相関した．
PG\_speech\_ratio（$\rho = 0.447$, $p < 0.0001$）およびPG\_overlap\_rate（$\rho = 0.500$, $p < 0.0001$）は年齢と比較的強い正の相関を示し，
年齢が高い話者ほど発話率・重なり率が高い傾向が確認された．
...
相互行為特徴量のうち13特徴量が年齢と，6特徴量が性別と有意な関連を示した．
...
A, N）で相互行為特徴量との有意な関連が認められた．具体的には，Agreeableness（A）
が$r = 0.449$（$p = 0.0004$, $p_{\text{corrected}} = 0.0020$）と5次元中で最も高
い予測精度を示した．Conscientiousness（C）が$r = 0.432$（$p = 0.0006$,
$p_{\text{corrected}} = 0.0024$）でこれに次いだ．Openness（O，$r = 0.410$, $p =
0.0014$, $p_{\text{corrected}} = 0.0042$），Neuroticism（N，$r = 0.317$, $p =
0.0122$, $p_{\text{corrected}} = 0.0244$）も有意であった．Extraversion（E）のみ
補正前・補正後ともに有意水準に達しなかった（$r = 0.234$, $p = 0.0658$,
$p_{\text{corrected}} = 0.0658$）．

\begin{table}[htbp]
\centering
\caption{仮想Big5に対する置換検定結果（全5次元）．
観測相関係数$r_{\text{obs}}$，補正前$p$値，およびHolm法による補正後$p$値を示す．$^{*}$は補正後$p < 0.05$．}
...
仮想Big5のCに対して有意であった特徴量は以下の5個である: PG\_speech\_ratio（発話
率，$\beta = 0.037$, $p = 0.015$），FILL\_has\_any（フィラー出現発話率，$\beta =
0.044$, $p = 0.006$），IX\_oirmarker\_after\_question\_rate（質問直後の修復開始率，
$\beta = 0.040$, $p = 0.030$），IX\_yesno\_rate（YES/NO応答率，$\beta = 0.042$,
$p = 0.007$），IX\_lex\_overlap\_mean（語彙的重なり，$\beta = -0.045$, $p =
0.032$）．有意な特徴量はPG系タイミング指標（1個），FILL系フィラー指標（1個），IX系
相互行為指標（3個）にまたがり，古典的特徴量（PG\_speech\_ratio，FILL\_has\_any）と
新規特徴量（IX系3個）の両群から同定された．一方，沈黙・応答の間合い系（PG\_pause系・
PG\_resp\_gap系）および応答型系（RESP系）の指標は有意水準に達しなかった．
...
各特徴量の回帰係数$\beta_{\text{obs}}$および$p$値を示す．$^{*}$は$p < 0.05$．}
...
徴量は以下の5個であり，置換検定で有意と判定された特徴量と完全に一致した:
PG\_speech\_ratio（$\bar{\beta} = 0.036$, CI $= [0.011, 0.062]$），
FILL\_has\_any（$\bar{\beta} = 0.043$, CI $= [0.020, 0.065]$），
IX\_oirmarker\_after\_question\_rate（$\bar{\beta} = 0.037$, CI $= [0.010, 0.063]$），
IX\_yesno\_rate（$\bar{\beta} = 0.040$, CI $= [0.013, 0.067]$），
IX\_lex\_overlap\_mean（$\bar{\beta} = -0.045$, CI $= [-0.081, -0.016]$）．
沈黙・応答の間合い系（PG\_pause系・PG\_resp\_gap系）および応答型系（RESP系）の指標は
...
置換検定ならびにBootstrap法の両方で有意・安定と判定された特徴量は以下の5個であり，両手法の結果は完全に一致した:
PG\_speech\_ratio（正の寄与），FILL\_has\_any（正の寄与），
IX\_oirmarker\_after\_question\_rate（正の寄与），IX\_yesno\_rate（正の寄与），
IX\_lex\_overlap\_mean（負の寄与）．これらの特徴量は古典的特徴量（PG\_speech\_ratio，FILL\_has\_any）と新規特徴量（IX系3個）の両群にまたがった．
...
両方で有意かつ安定と判定された共通の5特徴量，すなわち
PG\_speech\_ratio（発話率，正の寄与），
FILL\_has\_any（フィラー出現発話率，正の寄与），
IX\_oirmarker\_after\_question\_rate（質問直後の修復開始率，正の寄与），
IX\_yesno\_rate（YES/NO応答率，正の寄与），
IX\_lex\_overlap\_mean（語彙的重なり，負の寄与）が
Cの主要な関連因子として同定された．
発話量の多さ，および質問に対するYES/NO応答や修復開始の多さは
会話への積極的で明確な応答傾向を反映し，
Cの高い話者に想定される丁寧で関与的な対話行動と整合する．
一方，IX\_lex\_overlap\_mean（語彙的重なり）の負の寄与は，
直前の発話と語彙的に重なりすぎない応答がCの予測に寄与することを示す．
...
Permutationで有意な5特徴量はいずれもBootstrapの95\%CIがゼロを除外しており，
2つの手法の結果は完全に一致した．
これらは統計的有意性と安定性を兼ね備えた信頼性の高い寄与因子と解釈できる．
...
予測精度は平均$\Delta r = +0.026$とほぼ変化しなかった
（モデル別の値は\ref{sec:appendix_confound}参照）．
これは，特徴量とCの関連が性別・年齢を介した交絡では説明されないことを示す．
...
解釈すべきである．
年齢では13特徴量が有意に相関し，PG\_speech\_ratio（$\rho = 0.447$）・PG\_overlap\_rate（$\rho = 0.500$）は正，
沈黙系・応答遅れ系は負の相関を示した．
...
PG\_speech\_ratio（発話率），FILL\_has\_any（フィラー出現発話率），
IX\_oirmarker\_after\_question\_rate（質問直後の修復開始率），
IX\_yesno\_rate（YES/NO応答率），IX\_lex\_overlap\_mean（語彙的重なり）が
Cの主要な予測因子として同定された（Permutation有意かつBootstrap CIがゼロを除外する5共通特徴量）．
...
観測数が少ないと比率推定の標本変動が大きくなり，個々人の相互行為傾向を安定して反
映しなくなる．そこで，個々人の相互行為を反映できるような十分な会話量を担保するた
めに，以下の性質を有するデータのみを選定した．
...
モデルに依存しないように4つのLLMモデルから推定されたスコアを平均して用いた．

使用した4モデルは以下の通りである．


\begin{itemize}
  \item Anthropic Claude Sonnet 4（パラメータ数の規模は非公開）
  \item Alibaba Qwen3-235B（235Bパラメータ）
  \item DeepSeek DeepSeek-V3（671Bパラメータ）
  \item OpenAI GPT-OSS-120B（120Bパラメータ）
\end{itemize}
...
交絡変数を統制した後もCの有意性が維持されるかどうかを検証した．性別・年齢を説明変数に追加した場合の予測精度の変化（$\Delta r$）と$p$値の変化を報告する．Cについては，交絡統制前に有意であった3モデル（Claude Sonnet 4, Qwen3-235B, GPT-OSS-120B）はいずれも統制後も有意性を維持し，平均$\Delta r = +0.026$と予測精度はほぼ変化しなかった．DeepSeek-V3は統制前後ともに非有意であった．
