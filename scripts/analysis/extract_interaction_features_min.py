...
def main():
    ...
    utt=pd.read_parquet(args.utterances_parquet).copy()
    ...
    if "start_time" in utt.columns and "end_time" in utt.columns:
        ...
        utt["start_time"]=np.nan; utt["end_time"]=np.nan
    ...
    for conv_id,u in utt.groupby("conversation_id",sort=False):
        ...
        for spk,p in pairs.groupby("resp_speaker_id",sort=False):
            ...
            if us.empty:
                pg_speech_ratio=float("nan"); pause_mean=pause_p50=pause_p90=float("nan")
                text_all=""
            else:
                ...
                pauses=[x for x in pauses if x is not None and not math.isnan(x) and x>=args.gap_tol]
            ...
            overlap_rate=float((gaps<-args.gap_tol).mean()) if len(gaps)>0 else float("nan")
            gap_list=[float(g) for g in gaps if (not math.isnan(g)) and g>=args.gap_tol]
    ...
    if out.empty: raise SystemExit("No rows produced.")
