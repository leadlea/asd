...
<head>
  ...
  <style>
    ...
    .kv .num{ font-weight:900; color:#111; font-variant-numeric: tabular-nums; }
    ...
    .layout { display:grid; grid-template-columns: 1.05fr 0.95fr; gap:12px; margin-top:14px; align-items:start; }
...
def main():
    ...
    for _, r in df.iterrows():
        ...
        speaker_key = str(r.get("speaker_id") or "")
    ...
    data_json = _script_json(json.dumps(rows, ensure_ascii=False))
    stats_json = _script_json(json.dumps(stats, ensure_ascii=False))
    allowed_json = _script_json(json.dumps(allowed_labels, ensure_ascii=False))
