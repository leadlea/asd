...
<head>
  ...
  <style>
    ...
    /* __PCA_ONEPASS_V1__ */
    .pcaWrap{ border:1px solid var(--line); background:#fff; border-radius:14px; padding:14px; }
    .pcaMeta{ font-size:11px; color:var(--muted); margin:6px 0 8px; }
    .pcaSvg{ width:100%; height:520px; display:block; }
    .pcaLegend{ display:flex; flex-wrap:wrap; gap:8px; margin-top:8px; font-size:11px; color:var(--muted); }
    .pcaSw{ width:10px; height:10px; border-radius:3px; display:inline-block; margin-right:6px; border:1px solid var(--line); }

    /* layout: keep cards compact; PCA full width */
    .grid{ align-items: start; grid-auto-rows: auto; }
    .grid > .card{ height:auto; min-height: unset; align-self:start; }
    .card.cardPCA{ grid-column: 1 / -1; }
...
<body>
  <div class="wrap">
    ...
    <div class="grid">
      ...
      <div class="card cardPCA">
        <h3>Cluster map (PCA)</h3>
        <div class="panel" id="pcaBox"></div>
      </div>
    </div>
  ...
  <script>
    ...
    rows.forEach(r => {
      ...
      if (r.ix && typeof r.ix === "object") {
        Object.keys(r.ix).forEach(k => {
          if (k.startsWith("IX_") && r[k] === undefined) r[k] = r.ix[k];
        });
      }
    ...
    const els = {
      ...
      pcaBox: document.getElementById("pcaBox"),
    };
    ...
    // ---- PCA cluster map (requires CL_pca_x/y) ----
    function renderPcaBox(){
      if (!els.pcaBox) return;
      const pts = rows.filter(r => Number.isFinite(Number(r.CL_pca_x)) && Number.isFinite(Number(r.CL_pca_y)));
      if (!pts.length){
        els.pcaBox.innerHTML = '<div class="muted">No PCA points (CL_pca_x/y are missing).</div>';
        return;
      }
      const xs = pts.map(p => Number(p.CL_pca_x));
      const ys = pts.map(p => Number(p.CL_pca_y));
      const xmin = Math.min(...xs), xmax = Math.max(...xs);
      const ymin = Math.min(...ys), ymax = Math.max(...ys);
      const pad = 18, W = 520, H = 520;
      ...
      function sy(y){
        ...
        // y-axis inverted
        return pad + (ymax - y) * (H - 2*pad) / (ymax - ymin);
      }
      function col(c){
        const k = (c===null || c===undefined || c==="") ? "na" : String(c);
        const pal = {
          "0":"#60a5fa","1":"#34d399","2":"#fbbf24","3":"#f87171",
          "4":"#a78bfa","5":"#fb7185","na":"#9ca3af"
        };
        return pal[k] || "#9ca3af";
      }

      const clusters = Array.from(new Set(pts.map(p => (p.CL_fillpg_cluster===null||p.CL_fillpg_cluster===undefined) ? "na" : String(p.CL_fillpg_cluster)))).sort();
      const legend = clusters.map(k => `<span><span class="pcaSw" style="background:${col(k)}"></span>c${k}</span>`).join("");

      const circles = pts.map(p => {
        const x = sx(Number(p.CL_pca_x)).toFixed(2);
        const y = sy(Number(p.CL_pca_y)).toFixed(2);
        const c = col(p.CL_fillpg_cluster);
        return `<circle cx="${x}" cy="${y}" r="3.0" fill="${c}" fill-opacity="0.88" />`;
      }).join("");

      const svg =
        `<div class="pcaWrap">
          <div class="pcaMeta">points: ${pts.length} / ${rows.length} (colored by CL_fillpg_cluster)</div>
          <svg class="pcaSvg" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-hidden="true">
            <rect x="0" y="0" width="${W}" height="${H}" fill="#fff"></rect>
            <rect x="${pad}" y="${pad}" width="${W-2*pad}" height="${H-2*pad}" fill="#fafafa" stroke="#e5e7eb"></rect>
            ${circles}
          </svg>
          <div class="pcaLegend">${legend}</div>
        </div>`;
      els.pcaBox.innerHTML = svg;
    }
    ...
    // ---- init ----
    fillStats();
    try { renderPcaBox(); } catch(e) { console.warn("renderPcaBox failed", e); }
    fillSelectors();
    renderTable();
...
def main() -> None:
    ...
    ap.add_argument("--labels_parquet", default="", help="legacy: phase3 labels parquet")
    ap.add_argument("--fixed_parquet", default="", help="recommended: phase56 fixed parquet (single source of truth): file/dir or comma-separated")
    ...
    if args.fixed_parquet:
        fixed_paths = _collect_parquet_paths(args.fixed_parquet)
        if not fixed_paths:
            raise SystemExit(f"No parquet found for --fixed_parquet={args.fixed_parquet}")
        if len(fixed_paths) == 1:
            df = pd.read_parquet(fixed_paths[0])
        else:
            df = pd.concat([pd.read_parquet(p) for p in fixed_paths], ignore_index=True)
        mode = "fixed"
    else:
        if not args.labels_parquet:
            raise SystemExit("Either --labels_parquet or --fixed_parquet is required")
    ...
    pg_rows_loaded = 0
    pg_paths = [] if args.fixed_parquet else _collect_parquet_paths(args.pg_metrics_parquet)
    if pg_paths:
    ...
    ix_rows_loaded = 0
    ix_paths = [] if args.fixed_parquet else _collect_parquet_paths(args.ix_metrics_parquet)
    if ix_paths:
    ...
    for _, r in df.iterrows():
        ...
        if (not examples):
            examples = _safe_json_list_any(r.get("examples"))
        speaker_key = str(r.get("speaker_id") or "").strip().split("/")[-1]
        conv_key = str(r.get("conversation_id") or "").strip().split("/")[-1]
        if mode == "fixed" and speaker_key and (":" not in speaker_key) and conv_key:
            speaker_key = f"{conv_key}:{speaker_key}"


        # attach phase4 pg metrics
        if mode == "fixed":
            pg = {str(c): _safe_float(r.get(c)) for c in r.index if str(c).startswith("PG_")}
            pg = {k:v for k,v in pg.items() if v is not None}
        else:
            pg = pg_index.get(speaker_key, {}) if (pg_attached and speaker_key) else {}

        # attach interaction metrics (IX_*)
        if mode == "fixed":
            ix = {str(c): _safe_float(r.get(c)) for c in r.index if str(c).startswith("IX_")}
            ix = {k:v for k,v in ix.items() if v is not None}
        else:
    ...
    stats_obj = {
        ...
        "pg_attached": bool(pg_attached) or (mode=="fixed" and any(str(c).startswith("PG_") for c in df.columns)),
        "pg_rows_loaded": int(pg_rows_loaded) if mode!="fixed" else int(len(df)),
        "ix_attached": bool(ix_attached),
        "ix_rows_loaded": int(ix_rows_loaded),
        "pg_summary": pg_summary,
        "ix_attached": bool(ix_attached) or (mode=="fixed" and any(str(c).startswith("IX_") for c in df.columns)),
        "ix_rows_loaded": int(ix_rows_loaded) if mode!="fixed" else int(len(df)),
