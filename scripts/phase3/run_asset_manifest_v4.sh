...
LOCAL_OUT="${LOCAL_OUT:-artifacts/tmp_meta/asset_manifest_v4.parquet}"
OUT_S3="${OUT_S3:-$ANALYSIS_OUT/gold=v13/assets/asset_manifest_v4.parquet}"

echo "== asset_manifest v4 (CEJC variant-keyed) =="
...
python scripts/phase3/build_asset_manifest_v4.py \
...
echo "== quick check: CEJC wav coverage (base-normalized, raw vs manifest) =="
...
df=pd.read_parquet("artifacts/tmp_meta/asset_manifest_v4.parquet")

# manifest: base ids (conversation_id_base)
man=set(df[(df.dataset=="cejc") & (df.has_wav==True)].conversation_id_base.astype(str).str.upper().unique())

missing=sorted(raw-man)
print("raw wav base ids:", len(raw))
print("manifest has_wav base ids:", len(man))
